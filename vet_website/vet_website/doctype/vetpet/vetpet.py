...
def get_pet(filters=None):
	...
	try:
		pet = frappe.get_list("VetPet", filters=pet_filters, fields=["pet_name", "name", "parent", "register_date", "pet_description", "hewan_jenis", "status"], order_by=default_sort, limit=default_limit, start=(page - 1) * 10, page_length= 10)
		datalength = len(frappe.get_all("VetPet", filters=pet_filters, as_list=True))
