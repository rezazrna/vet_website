...
from vet_website.vet_website.doctype.vetppurchase.vetpetowner import set_owner_credit_total
...
def get_pet_owner(filters=None):
	...
	try:
		owner = frappe.get_list("VetPetOwner", filters=owner_filters, fields=['name', 'nik', 'owner_name', 'phone', 'email', 'address', 'creation'], order_by=default_sort, limit=default_limit)
...
def get_credit_list(name=False, no_filter=False, filters=None, supplier=False):
	...
	try:
		...
		if no_filter == 'true':
			if supplier == 'true':
				owner_credit_list = frappe.get_list('VetOwnerCredit', or_filters={'supplier': ['!=', ''], 'purchase': ['!=', '']}, filters=credit_filters, fields=['*'], order_by=default_sort)
				...
				if session not in ['',False]: 
					...
					credit_filters.append(['date', '<=', doc_session.closing_session or datetime.datetime.today()])
				...
				owner_credit_list = frappe.get_list('VetOwnerCredit', or_filters={'pet_owner': ['!=', ''], 'invoice': ['!=', '']}, filters=credit_filters, fields=['*'], order_by=default_sort)
				owner_list = frappe.get_list('VetPetOwner', fields=['*'])
		else:
			owner_credit_list = frappe.get_list('VetOwnerCredit', or_filters={'pet_owner': name, 'supplier': name}, filters=credit_filters, fields=['*'], order_by=default_sort)
			owner_list = []
		...
		return {'owner_credit_list': owner_credit_list, 'owner_list': owner_list, 'payment_method_list': payment_method}
	except PermissionError as e:
		return {'error': e}

@frappe.whitelist()
def reception_get_pet_owner(nik=False, nip=False):
	try:
		...
		if o['type'] == 'Payment' or o['type'] == 'Refund' or o['type'] == 'Cancel':
			if o['nominal'] < 0 or owner_credit.is_deposit:
				...
				if len(purchase_search) > 0:
					...
					total = sum([(p.quantity_receive*p.price)-(p.quantity_receive*p.price*(p.discount/100)) for p in purchase.products])
					print(paid)
					print(total)
					print(excess)
					# if paid > total:
					# 	print("Paid lebih besar dari total")
					# 	owner_credit.credit = 0
					# 	credit += excess
					# 	owner_credit.credit = credit
			...
			if o['type'] == 'Sales':
				...
				if len(prev_payment_search) > 0:
					...
					if prev_payment.credit >= o['nominal']:
						...
						owner_credit.credit = 0
						credit -= prev_payment.credit
						owner_credit.credit = credit
						owner_credit.credit_mutation = -prev_payment.credit
						
						owner_credit.debt = 0
						debt += (o['nominal'] - prev_payment.credit)
						owner_credit.debt = debt
						owner_credit.debt_mutation = (o['nominal'] - prev_payment.credit)
				else:
