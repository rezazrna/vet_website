...
def get_pet(name):
	...
	try:
		...
		for o in owner:
			...
			o.update({'pets': pets, 'total_spending': total_spending, 'total_visit': total_visit, 'petType': petTypeAll, 'total_credit': total_credit, 'total_debt': total_debt})
		return owner
...
def get_reception_list(filters=None):
	...
	try:
		reception = frappe.get_list("VetReception", filters=reception_filters, or_filters=reception_or_filters, fields=["reception_date", "description", "name", "pet", "owner", "service", "queue", "register_number"], order_by=default_sort)
