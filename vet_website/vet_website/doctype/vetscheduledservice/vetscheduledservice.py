...
def get_scheduled_service_list(filters=None):
	...
	try:
		scheduled = frappe.get_list("VetScheduledService", filters=scheduled_filters, fields=["create_date", "pet", "pet_name", "pet_owner_name", "service", "schedule_date", "status", "register_number", "pet_owner_phone", "type_name", "name"], order_by=default_sort, start=(page - 1) * 10, page_length= 10)
		datalength = len(frappe.get_all("VetScheduledService", filters=scheduled_filters, as_list=True))
