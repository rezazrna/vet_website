...
class Grooming extends React.Component {
    constructor(props) {
        ...
        this.groomingSearch = this.groomingSearch.bind(this);
    ...
    componentDidMount() {
        ...
        if (sessionStorage.getItem(window.location.pathname) != null && (document.referrer.includes('/main/dokter-dan-jasa/grooming/detail') || document.referrer.includes('/main/dokter-dan-jasa/grooming/edit'))) {
        ...
        frappe.call({
            type: "GET",
            method: "vet_website.vet_website.doctype.vetgrooming.vetgrooming.get_grooming_list",
            args: { filters: new_filters },
            callback: function (r) {
                if (r.message) {
                    console.log(r.message);
                    po.setState({ 'data': r.message.grooming, 'loaded': true, 'datalength': r.message.datalength });
                }
            }
        });
    }
    ...
    groomingSearch(filters) {
        ...
        frappe.call({
            ...
            callback: function (r) {
                if (r.message) {
                    console.log(r.message);
                    po.setState({ 'data': r.message.grooming, 'datalength': r.message.datalength, 'loaded': true });
                }
            }
        });
    }
    ...
    deleteRow(e) {
        ...
        frappe.call({
            type: "GET",
            method: "vet_website.vet_website.doctype.vetgrooming.vetgrooming.delete_grooming",
    ...
    render() {
        ...
        var sorts = [
            ...
            // 	{'label': 'No Antrian DESC', 'value': 'name desc'},
            // 	{'label': 'No Antrian ASC', 'value': 'name asc'},
            ...
            return (
                <div>
                    <div className="row mx-0" style={row_style2}>
                        ...
                        <div className="col">
                            <input value={this.state.search || ''} className="form-control fs12" name="search" placeholder="Search..." style={formStyle} onChange={e => this.setState({ search: e.target.value })} onKeyDown={(e) => e.key === 'Enter' ? this.groomingSearch(JSON.parse(sessionStorage.getItem(window.location.pathname))) : null} />
                        </div>
                        <div className="col-7">
                            <Filter sorts={sorts} searchAction={this.groomingSearch} field_list={field_list} filters={JSON.parse(sessionStorage.getItem(window.location.pathname))} />
                        </div>
                    </div>
                    <GroomingList grooming={this.state.data} checkRow={this.checkRow} checkAll={() => this.checkAll()} check_all={this.state.check_all} paginationClick={this.paginationClick} currentpage={this.state.currentpage} datalength={this.state.datalength} />
                </div>
            )
        }
        else {
...
class GroomingList extends React.Component {
    render() {
        ...
        //     }
        //     var fields = [row.reception_date, row.queue, row.pet, row.pet_name, row.pet_owner.owner_name, row.description, row.status]
        ...
        if (grooming.length != 0) {
            ...
            // currentItems = grooming.filter(filterRow).slice(indexOfFirstTodo, indexOfLastTodo):
            // currentItems = grooming.slice(indexOfFirstTodo, indexOfLastTodo)
            grooming.forEach(function (grooming, index) {
                // if (currentItems.includes(grooming)){
                grooming_rows.push(
                    <GroomingListRow key={grooming.name} grooming={grooming} checkRow={() => pol.props.checkRow(index)} />
                )
                // }
            })
...
class GroomingListRow extends React.Component {
    ...
    render() {
        var checked = false
        if (this.props.grooming.checked) {
            checked = true
        }
        var statusClass
        var grooming = this.props.grooming
        ...
        } else if (grooming.status == 'Done') {
            statusClass = 'bg-success'
        } else if (grooming.status == 'Cancel') {
            ...
            <div className="row mx-0">
                ...
                <div className="col row-list row-list-link" onClick={() => this.clickRow()}>
                    <div className="row mx-0 fs12 fw600">
                        ...
                        <div className="col d-flex">
                            <span className="my-auto">{grooming.queue}</span>
                        </div>
                        <div className="col d-flex">
                            <span className="my-auto">{grooming.pet}</span>
                        </div>
                        <div className="col-2 d-flex">
                            <span className="my-auto">{grooming.pet_name}</span>
                        </div>
                        <div className="col-2 d-flex">
                            <span className="my-auto">{grooming.pet_owner.owner_name}</span>
                        </div>
                        <div className="col d-flex">
                            <span className="my-auto">{grooming.description}</span>
                        </div>
                        <div className="col-1 d-flex">
...
ReactDOM.render(<Grooming />, document.getElementById('grooming_list'))