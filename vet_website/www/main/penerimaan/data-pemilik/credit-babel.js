            ...
            return (
                <div>
                    <div className="row mx-0" style={row_style}>
                        ...
                        <div className="col">
                            <input value={this.state.search || ''} className="form-control fs12" name="search" placeholder="Search..." style={formStyle} onChange={e => this.setState({ search: e.target.value })} onKeyDown={(e) => e.key === 'Enter' ? this.creditSearch(JSON.parse(sessionStorage.getItem(window.location.pathname))) : null} />
                        </div>
                        <div className="col-5">
...
class CreditList extends React.Component {
    render() {
        ...
        console.log(this.props.mode)
        if (!this.props.supplier) {
            !this.props.mode ? data_names = data
                // .filter(filterRow)
                .filter(d => d.type == 'Payment' && d.nominal >= 0).map(d => d.name) :
                this.props.mode == 'debt' ? data_names = data
                    // .filter(filterRow)
                    .filter(d => d.debt_mutation != 0).map(d => d.name) :
                    this.props.mode == 'credit' ? data_names = data
                        // .filter(filterRow)
                        .filter(d => d.credit_mutation != 0).map(d => d.name) : false
        } else {
            !this.props.mode ? data_names = data
                // .filter(filterRow)
                .filter(d => d.type == 'Payment' && d.nominal >= 0).map(d => d.name) :
